...
|                                                              |
|  Single unified route ML pipeline:                           |
|  Upload CSV → run all processing in ONE route                |
|  Uses existing pre_process.py + data_processing.py           |
+==============================================================+
"""
...
# Create FastAPI app
app = FastAPI(
    title="Unified ML Pipeline API",
    description="Single route: Upload CSV → Full ML pipeline (EDA + preprocessing + training)",
    version="1.0.0"
)
...
CSV_DATA_DIR = Path(PROJECT_DIR) / "csv_data"
...
def run_pipeline_unified(csv_path: str, target_column: str = None):
    """
    UNIFIED SINGLE ROUTE: Run complete ML pipeline
    Uses existing functions from pre_process.py, data_processing.py, train_model.py
    ...
    try:
        ...
        print(f"\n[OK] Loaded: {os.path.basename(csv_path)} ({df.shape[0]} rows x {df.shape[1]} cols)")
        ...
        print(f"[DETECT] Task type: {task_type.upper()}")
        
        # STEP 1: AI Analysis (uses pre_process.py)
        print("\n[STEP 1/3] Running AI Data Analysis...")
        report_path = run_analysis(df, target_column, task_type, PROJECT_DIR, GEMINI_MODEL)
        
        # STEP 2: AI Preprocessing (uses data_processing.py)
        ...
        # STEP 3: Model Training (uses train_model.py)
        print("[STEP 3/3] Training Models...")
        run_training(clean_df, target_column, task_type, review, PROJECT_DIR)
        
        # Load results from JSON files generated by run_training
        results = {
            "status": "success",
            "task_type": task_type,
        ...
        try:
            with open(os.path.join(PROJECT_DIR, "analysis_report.txt"), "r", encoding="utf-8", errors="ignore") as f:
...
def health_check():
    ...
    return {
        "status": "✓ healthy",
        "service": "Unified ML Pipeline API",
        "version": "1.0.0"
    }
...
def list_files():
    ...
    try:
        csv_files = [f for f in os.listdir(CSV_DATA_DIR) if f.endswith(".csv")]
        return {
            "csv_data_folder": str(CSV_DATA_DIR),
            "files": csv_files,
            "count": len(csv_files)
        }
    except Exception as e:
...
async def predict(
    ...
    target_column: str = Query(None, description="Target column name (optional, defaults to last column)")
):
    """
    Upload a CSV file and run the COMPLETE ML pipeline in ONE unified route.
    
    This endpoint runs:
    1. AI Data Analysis (pre_process.py)
    2. AI Preprocessing (data_processing.py)
    ...
    try:
        ...
        print(f"\n{'='*60}")
        print(f"[API] New Pipeline Run: {file.filename}")
        print(f"{'='*60}")
        
        # Run unified pipeline
        results = run_pipeline_unified(str(file_path), target_column)
...
async def predict_from_path(
    file_name: str = Query(..., description="CSV file name in csv_data/ folder"),
    target_column: str = Query(None, description="Target column name (optional, defaults to last column)")
):
    """
    ...
    Returns:
        Complete pipeline results (same as /predict endpoint)
    """
    
    try:
        full_path = CSV_DATA_DIR / file_name
        ...
        print(f"\n{'='*60}")
        print(f"[API] Pipeline Run (from csv_data): {file_name}")
        print(f"{'='*60}")
        
        # Run pipeline
        results = run_pipeline_unified(str(full_path), target_column)
